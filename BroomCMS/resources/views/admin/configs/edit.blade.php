    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.configs.update", [$config->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nom') ? 'is-invalid' : '' }}" type="text" name="nom" id="nom" value="{{ old('nom', $config->nom) }}" required>
                @if($errors->has('nom'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('gabarits') ? 'is-invalid' : '' }}" name="gabarits[]" id="gabarits" multiple required>
                    @foreach($gabarits as $id => $gabarit)
                        <option value="{{ $id }}" {{ (in_array($id, old('gabarits', [])) || $config->gabarits->contains($id)) ? 'selected' : '' }}>{{ $gabarit }}</option>
                    @endforeach
                </select>
