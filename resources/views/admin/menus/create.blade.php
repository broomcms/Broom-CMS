    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.menus.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('titre') ? 'is-invalid' : '' }}" type="text" name="titre" id="titre" value="{{ old('titre', '') }}" required>
                @if($errors->has('titre'))
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Menu::TARGET_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('target') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="target_{{ $key }}" name="target" value="{{ $key }}" {{ old('target', '') === (string) $key ? 'checked' : '' }}>
