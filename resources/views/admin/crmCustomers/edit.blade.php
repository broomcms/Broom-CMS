...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.crm-customers.update", [$crmCustomer->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="utilisateur_id">{{ trans('cruds.crmCustomer.fields.utilisateur') }}</label>
                <select class="form-control select2 {{ $errors->has('utilisateur') ? 'is-invalid' : '' }}" name="utilisateur_id" id="utilisateur_id">
                    @foreach($utilisateurs as $id => $utilisateur)
                        <option value="{{ $id }}" {{ (old('utilisateur_id') ? old('utilisateur_id') : $crmCustomer->utilisateur->id ?? '') == $id ? 'selected' : '' }}>{{ $utilisateur }}</option>
                    @endforeach
                </select>
                @if($errors->has('utilisateur'))
                    <span class="text-danger">{{ $errors->first('utilisateur') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.crmCustomer.fields.utilisateur_helper') }}</span>
            </div>
            <div class="form-group">
